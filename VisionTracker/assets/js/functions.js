$(function () {
  ...
  if ($body.attr("data-preloader") === "1") {
    ...
    $body.append($("<div class='preloader preloader-8'><div><h6 class='heading-uppercase'>Loading</h6><span class='spinner'></span></div></div>"));
  }
  else if ($body.attr("data-preloader") === "9") {
    $body.append($("<div class='preloader preloader-9'><div><span class='spinner'><span></div></div>"));
  }
  ...
  //VT making :Contains case insensitive
  ...
  document.resetAllAccordions = resetAccordions;

  var spotlightAnimations = {
    duration: "fast"
  }

  function unspotlight() {
    $(".accordion").slideDown({ duration: "fast" });
    $(".accordion-content").parent().slideDown({ duration: "fast" });
    $(".milestone").slideDown(spotlightAnimations);
  }

  function openAccordionsContaining(query) {
    //resetAccordions();
    /*   
    $(".accordion").slideUp({duration: "slow"});
    $(".accordion-content").parent().slideUp({duration: "fast"});
    $(".milestone").slideUp(spotlightAnimations);  
     */

    $(".accordion:not(.accordion:contains('" + query + "'))").each(function () {
      var $this = $(this);
      $this.slideUp({ duration: "fast" });
    });

    $(".accordion li:not(.accordion li:contains('" + query + "'))").each(function () {
      var $this = $(this);
      $this.slideUp({ duration: 400 });
    });

    $(".milestone:not(.milestone:contains('" + query + "'))").each(function () {
      var $this = $(this);
      $this.closest(".milestone").slideUp({ duration: 300 });
      $this.closest(".milestone").fadeOut({ duration: 300 });
    });


    $(".milestone:contains('" + query + "')").each(function () {
      var $this = $(this);
      $this.closest(".accordion").slideDown({ duration: 200 });
      setTimeout(function () { $this.closest(".accordion-content").parent().slideDown({ duration: 400 }); }, 400);
      //setTimeout(function() { $this.closest(".milestone").slideDown({duration: 100});}, 1000);
      ;
    });

    $(".milestone:contains('" + query + "')").each(function () {
      var $this = $(this);
      var element = $this.parent().parentsUntil("li").parent()[0];
      openAccordion(element, true);
    });
  }
  document.openAccordionsContaining = openAccordionsContaining;

  function closeAllAccordions() {
    $(".accordion-oneopen").find(".active").removeClass("active");
    $(".accordion-oneopen").find(".accordion-content").css("max-height", "0");
  }

  function openAccordion(element, searchMode = false) {
    var $element = $(element);
    var $this = $element.find(".accordion-title");

    var accordionList = $this.parent("li");
    //var accordionContent = element.nextElementSibling;
    var accordionContent = $this.next()[0];

    if (accordionList.hasClass("active") && !searchMode) {
      ...
      //VT-start
      if (!searchMode) closeAllAccordions();
      //VT-end

      accordionList.addClass("active");

      if ($this.closest(".accordion").hasClass("accordion-oneopen") && !searchMode) {
  ...
  /*===============================================
    Accordion VT SEGMENTED PROGRESS
  ===============================================*/

  $(".accordion-title").append("<div class='segmented-progress-bar'></div>");
  $(".milestones").each(function () {
    var $this = $(this);
    var totalMilestones = $this.find(".milestone").length;
    var totalCompletedMilestones = $this.find(".milestone.complete").length;
    var totalIncompletMilestones = $this.find(".milestone.incomplete").length;
    var segmentSize = 100 / totalMilestones;
    //var marginPercent = 0.03;
    //var segmentMargin = segmentSize * marginPercent;
    //segmentSize = segmentSize - segmentMargin;
    var segmentMargin = 0;
    var segmentStyle = "width:" + segmentSize + "%; margin-right:" + segmentMargin + "%;";

    var i = 0;

    for (i = 0; i < totalCompletedMilestones; i++) {
      $this.closest("li").find(".segmented-progress-bar").append("<div style='" + segmentStyle + "' class='segment completed'></div>");
    }

    for (i = 0; i < totalIncompletMilestones; i++) {
      $this.closest("li").find(".segmented-progress-bar").append("<div style='" + segmentStyle + "' class='segment'></div>");
    }

    //$this.closest("li").find(".chip.state").text(totalCompletedMilestones + " of " + totalMilestones);
    $this.closest("li").find(".chip.state").text(Math.round((totalCompletedMilestones / totalMilestones) * 100) + "%");
